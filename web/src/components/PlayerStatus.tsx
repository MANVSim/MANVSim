...
import {ExecutionStatus, Player} from "../types"
...
export function PlayerStatus({player, exec_status}: PlayerStatusProps) {
    ...
    return (
        <tr>
            ...
            <td>
                {player.logged_in ? (
                    player.alerted ? (
                        <Button className="btn btn-outline-primary w-50" title="Spieler wurde bereits alamiert."
                                disabled>
                            Alarmiert
                        </Button>
                    ) : (
